...
class CalculateRequiredPower(Action):
    ...
    def perform(self, db, part_ids):
        ...
        drive = db.session.query(part_tables.get('drives')).filter_by(id=drive_id).first()
        random_access_memory = db.session.query(part_tables.get('random_access_memory')).filter_by(id=ram_id).first()
        graphics_card = db.session.query(part_tables.get('graphics_cards')).filter_by(id=gpu_id).first()
