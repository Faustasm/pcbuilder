...
class CheckPartCompatability(Action):
    ...
    def perform(self, db, part_ids):
        ...
        motherboard = db.session.query(part_tables.get('motherboards')).filter_by(id=mb_id).first()
        processor = db.session.query(part_tables.get('processors')).filter_by(id=cpu_id).first()
        random_access_memory = db.session.query(part_tables.get('random_access_memory')).filter_by(id=ram_id).first()
        ...
        if processor and random_access_memory:
            ...
            if processor.max_memory_supported_gb != random_access_memory.total_capacity_gb:
        ...
        if motherboard and random_access_memory:
            ...
            if motherboard.max_memory_supported_gb != random_access_memory.total_capacity_gb:
        ...
        return {
            "compatability_issues": compatability_issues
        }
