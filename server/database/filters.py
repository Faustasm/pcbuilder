    ...
    for col in resource_table.c:
        ...
        if filter_name not in ignored_resource_filter_fields:
            if filter_name in shared_parent_table_names:
                parent_table_name = shared_parent_table_names.get(
                    filter_name
                )
                parent_table = resource_parent_tables.get(
                    parent_table_name
                )
                key = 'name' if 'name' in parent_table.c else 'model'
                q = db.session.query(parent_table.c.id, parent_table.c[key]).distinct().join(
                    resource_table,
                    parent_table.c.id==col
                ).all()
                ...
                parent_table_name = individual_parent_table_names.get(
                    filter_name
                )
