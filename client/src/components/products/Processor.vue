        ...
        <div class="w3-card-1">
         <div class="w3-container w3-center">
           <img
              ...
              class="w3-margin-top"
              :src="require(`../../assets/products/cpu/${processorData.image_url}.jpg`)"
              height="100"
            />
           <img
              v-else
           ...
           <p>
             <img
                class="w3-margin-top"
                :src="require(`../../assets/brand_icons/${processorData.brands}.svg`)"
                height="15"
              />
           </p>
           <p>
           ...
           <p v-if="!short">
             {{$t('components.products.processor.description.maxMemorySupported')}}:
             {{processorData.max_memory_supported}}
           </p>
           <p v-if="!short">
             {{$t('components.products.processor.description.integratedGraphicsCard')}}:
           </p>
           <p v-if="!short">
             {{processorData.integrated_graphics || 'Nėra'}}
           </p>
           <p v-if="!short">
             {{$t('components.products.processor.description.operatingFrequency')}}:
...
export default {
  ...
  async created() {
    if (this.id) {
      await axios
        .get(
          'http://127.0.0.1:5000/parts/processors',
          {
            params: {'id': this.id}
          }
        )
